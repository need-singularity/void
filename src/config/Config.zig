...
// like the `void-build-data` binary depend on the Config but don't
...
pub const compatibility = std.StaticStringMap(
    ...
    // Void 1.1 introduced background-blur support for Linux which
    ...
    // Void 1.2 renamed all our adw options to gtk because we now have
    // a hard dependency on libadwaita.
    .{ "adw-toolbar-style", cli.compatibilityRenamed(Config, "gtk-toolbar-style") },

    // Void 1.2 removed the `hidden` value from `gtk-tabs-location` and
    // moved it to `window-show-tab-bar`.
    .{ "gtk-tabs-location", compatGtkTabsLocation },

    // Void 1.2 lets you set `cell-foreground` and `cell-background`
    ...
    // Void 1.2 merged `bold-is-bright` into the new `bold-color`
    // by setting the value to "bright".
    .{ "bold-is-bright", compatBoldIsBright },

    // Void 1.2 removed the "desktop" option and renamed it to "detect".
    // The semantics also changed slightly but this is the correct mapping.
    .{ "gtk-single-instance", compatGtkSingleInstance },

    // Void 1.3 rename the "window" option to "new-window".
    // See: https://github.com/ghostty-org/ghostty/pull/9764
    .{ "macos-dock-drop-behavior", compatMacOSDockDropBehavior },
});

/// Set Void's graphical user interface language to a language other than the
/// system default language. For example:
///
///     language = de
///
/// will force the strings in Void's graphical user interface to be in German
/// rather than the system default.
///
/// This will not affect the language used by programs run _within_ Void.
/// Those will continue to use the default system language. There are also many
/// non-GUI elements in Void that are not translated - this setting will have
/// no effect on those.
///
/// Warning: This setting cannot be reloaded at runtime. To change the language
/// you must fully restart Void.
///
/// GTK only.
...
/// etc.
///
/// Notes on emoji specifically: On macOS, Void by default will always use
...
/// found, Void will use the regular style. This prevents falling back to a
...
/// set `font-family-bold = FooBar` and "FooBar" cannot be found, Void will
/// use whatever font is set for `font-family` for the bold style.
///
/// Finally, some styles may be synthesized if they are not supported.
/// For example, if a font does not have an italic style and no alternative
/// italic font is specified, Void will synthesize an italic style by
...
/// Control whether Void should synthesize a style if the requested style is
/// not available in the specified font-family.
///
/// Void can synthesize bold, italic, and bold italic styles if the font
...
///
/// The font feature will apply to all fonts rendered by Void. A future
...
///
/// Void breaks runs at certain points to improve readability or usability.
/// For example, Void by default will break runs under the cursor so that
...
///
/// This configuration only applies to Void builds that use FreeType.
...
///     This is the most common setting in GTK apps and therefore also Void's
...
/// name, or an absolute path to a custom theme file. Void also supports
/// specifying a different theme to use for light and dark mode. Each
/// option is documented below.
///
/// If the theme is an absolute pathname, Void will attempt to load that
...
///
/// The first directory is the `themes` subdirectory of your Void
/// configuration directory. This is `$XDG_CONFIG_HOME/void/themes` or
/// `~/.config/void/themes`.
///
/// The second directory is the `themes` subdirectory of the Void resources
/// directory. Void ships with a multitude of themes that will be installed
/// into this directory. On macOS, this list is in the
/// `Void.app/Contents/Resources/void/themes` directory. On Linux, this
/// list is in the `share/void/themes` directory (wherever you installed the
/// Void "share" directory.
///
/// To see a list of available themes, run `void +list-themes`.
///
/// A theme file is simply another Void configuration file. They share
...
/// `config-file`. At the time of writing this, Void will not show any
...
/// splits. A future improvement to Void will address this.
///
...
/// memory usage (specifically VRAM usage). A future Void improvement
...
///
/// If this is not set at all (`null`), then Void will respect DEC Mode 12
...
/// Void's shell integration features to be enabled.
///
...
///   anything but modifiers or keybinds that are processed by Void).
///
...
/// to all scrolling devices. Specifying a prefix was introduced in Void
/// 1.2.1.
///
...
///
/// On macOS, changing this configuration requires restarting Void completely.
...
/// When `true`, inactive cells in grid mode are dimmed using
/// `unfocused-split-opacity` / `unfocused-split-fill`. When `false` (the
/// default), the dimming overlay is skipped and every cell renders at full
/// opacity. The runtime toggle in the macOS "Grid" menu writes this value.
@"grid-dim-inactive": bool = false,
...
/// Control when Void preserves a zoomed split. Under normal circumstances,
...
/// created when Void starts, use the `initial-command` configuration.
///
/// Void supports the common `-e` flag for executing a command with
/// arguments. For example, `void -e fish --with --custom --args`.
/// This flag sets the `initial-command` configuration, see that for more
/// information.
command: ?Command = null,

/// This is the same as "command", but only applies to the first terminal
/// surface created when Void starts. Subsequent terminal surfaces will use
...
///
/// If you're using the `void` CLI there is also a shortcut to set this
/// with arguments directly: you can use the `-e` flag. For example: `void -e
...
///
///   * `quit-after-last-window-closed=true` - This ensures that the Void
...
/// variables set by Void. For example, if you set `VOID_RESOURCES_DIR`
/// then the value you set here will override the value Void typically
/// automatically injects.
///
/// These environment variables _will not_ be passed to commands run by Void
...
/// The title Void will use for the window. This will force the title of the
/// window to be this title at all times and Void will ignore any set title
...
/// bus name that Void uses to connect to DBus.
///
/// Note that changing this value between invocations will create new, separate
/// instances, of Void when running with `gtk-single-instance=true`. See that
/// option for more details.
///
/// Changing this value may break launching Void from `.desktop` files, via
/// DBus activation, or systemd user services as the system is expecting Void
/// to connect to DBus using the default `class` when it is launched.
///
/// The class name must follow the requirements defined [in the GTK
/// documentation](https://docs.gtk.org/gio/type_func.Application.id_is_valid.html).
///
/// The default is `com.dancinlab.void`.
///
...
/// setting. If a previous Void terminal exists in the same process,
...
/// if Void can detect it is launched from launchd (double-clicked) or
/// `open`, then it defaults to `home`. On Linux with GTK, if Void can detect
...
/// the documentation or using the `void +list-actions` command.
///
...
/// otherwise bound. When looking up a binding, Void first tries to match
...
///
/// Warning: If you define a sequence as a CLI argument to `void`,
/// you probably have to quote the keybind since `>` is a special character
/// in most shells. Example: void --keybind='ctrl+a>n=new_window'
///
/// A trigger sequence has some special handling:
///
///   * Void will wait an indefinite amount of time for the next key in
...
///  * `global:`
///
///    Make the keybind global. By default, keybinds only work within Void
...
///    (e.g. even when Void is not focused), specify this prefix.
...
///    if there is a selection to copy. If there is no selection, Void
...
/// to Void. When a `global:` keybind is specified and Void is launched
/// or reloaded, Void will attempt to request these permissions.
...
/// sequence.
///
/// Chained keybinds are available since Void 1.3.0.
///
/// ## Key Tables
///
...
///
/// Key tables are available since Void 1.3.0.
keybind: Keybinds = .{},

/// Remap modifier keys within Void. This allows you to swap or reassign
...
///
/// This will NOT change keyboard layout or key encodings outside of Void.
...
/// before Void receives the key events.
///
/// Example:
///
...
///   prior to Void receiving them. For example, `option+a` on macOS
...
///   this happens before Void receives the key event. To workaround
...
/// may override this preference. Void will do its best to respect this
...
///    desktop environment. This option usually makes Void look the most
...
///   * `void` - Use the background and foreground colors specified in the
///     Void configuration. This is only supported on Linux builds.
///
/// On macOS, if `macos-titlebar-style` is `tabs` or `transparent`, the window theme will be
/// automatically set based on the luminosity of the terminal background color.
/// This only applies to terminal windows. This setting will still apply to
/// non-terminal windows within Void.
///
...
/// certain size to fit within the grid. There is nothing Void will do about
...
/// to be a certain position to fit within the grid. There is nothing Void
...
///
///   * `always` will always save window state whenever Void is exited.
///
/// If you change this value to `never` while Void is not running, the next
/// Void launch will NOT restore the window state.
///
/// If you change this value to `default` while Void is not running and the
/// previous exit saved state, the next Void launch will still restore the
/// window state. This is because Void cannot know if the previous exit was
/// due to a forced save or not (macOS doesn't provide this information).
///
/// If you change this value so that window state is saved while Void is not
/// running, the previous window state will not be restored because Void only
...
/// window-theme is set to void. Currently only supported in the GTK app
/// runtime.
///
/// Specified as either hex (`#RRGGBB` or `RRGGBB`) or a named X11 color.
...
/// config path ($XDG_CONFIG_HOME/void/config.void).
///
/// If this is false, the default configuration paths will not be loaded.
/// This is targeted directly at using Void from the CLI in a way
...
/// Void will quit immediately after the last window is closed if
/// `quit-after-last-window-closed` is `true`.
///
/// Only implemented on Linux.
@"quit-after-last-window-closed-delay": ?Duration = null,

/// This controls whether an initial window is created when Void
/// is run. Note that if `quit-after-last-window-closed` is `true` and
/// `quit-after-last-window-closed-delay` is set, setting `initial-window` to
/// `false` will mean that Void will quit after the configured delay if no
...
///
/// On macOS, changing this configuration requires restarting Void
/// completely.
///
...
///
/// Available since: 1.2.0
@"gtk-quick-terminal-namespace": [:0]const u8 = "void-quick-terminal",
...
/// Void uses the compositor-reported primary output when available and
...
///     converts TERM from `xterm-void` to `xterm-256color` when connecting to
...
///     Attempts to install Void's terminfo entry using `infocmp` and `tic` when
...
///     (Available since: 1.2.0)
///
///   * `path` - Add Void's binary directory to PATH. This ensures the `void`
...
/// when both `ssh-env` and `ssh-terminfo` are enabled, Void will install its
/// terminfo on remote hosts and use `xterm-void` as TERM, falling back to
...
/// command-palette-entry = title:"Void",description:"Add a little Void to your terminal.",action:"text:\xf0\x9f\x91\xbb"
/// ```
///
...
/// Void currently supports OSC 10 (foreground), OSC 11 (background), and
...
///
/// Warning: Invalid shaders can cause Void to become unusable such as by
...
/// largely match Shadertoy, with some caveats and Void-specific extensions.
///
...
///
///    Request the user's attention when Void is unfocused, until it has
...
///      bring the Void window into focus;
///
...
/// Control the in-app notifications that Void shows.
///
...
/// and not something Void enables.
///
...
/// onto the Void icon in the macOS dock.
///
/// Valid values are:
///
...
/// If true, Void on macOS will automatically enable the "Secure Input"
/// feature when it detects that a password prompt is being displayed.
///
/// "Secure Input" is a macOS security feature that prevents applications from
/// reading keyboard events. This can always be enabled manually using the
/// `Void > Secure Keyboard Entry` menu item.
///
...
/// If true, Void will show a graphical indication when secure input is
...
/// If true, Void exposes and handles the built-in AppleScript dictionary
/// on macOS.
///
...
/// Valid values:
///
///  * `official` - Use the official Void icon.
///  * `blueprint`, `chalkboard`, `microchip`, `glass`, `holographic`,
///    `paper`, `retro`, `xray` - Official variants of the Void icon
...
///  * `custom-style` - Use the official Void icon but with custom
...
///
/// Defaults to `~/.config/void/Void.icns`
...
/// Whether macOS Shortcuts are allowed to control Void.
///
/// Void exposes a number of actions that allow Shortcuts to
/// control and interact with Void. This includes creating new
...
/// Valid values are:
///
/// * `ask` - Ask the user whether for permission. Void will remember
///   this choice and never ask again. This is similar to other macOS
///   permissions such as microphone access, camera access, etc.
///
/// * `allow` - Allow Shortcuts to control Void without asking.
///
/// * `deny` - Deny Shortcuts from controlling Void.
///
/// Available since: 1.2.0
...
/// instead of the entire Void process. Similarly, if a shell program is
/// using too much CPU, only that surface will be CPU-throttled.
///
/// This will cause startup times to be slower (a hundred milliseconds or so),
/// so the default value is "single-instance." In single-instance mode, only
/// one instance of Void is running (see gtk-single-instance) so the startup
/// time is a one-time cost. Additionally, single instance Void is much
/// more likely to have many windows, tabs, etc. so cgroup isolation is a
/// big benefit.
///
/// This feature requires `systemd`. If `systemd` is unavailable, cgroup
/// initialization will fail. By default, this will not prevent Void from
...
///   * `single-instance` - Enable cgroups only for Void instances launched
...
/// If `true`, the Void GTK application will run in single-instance mode:
/// each new `void` process launched will result in a new window if there is
/// already a running process.
///
/// If `false`, each new void process will launch a separate application.
///
/// If `detect`, Void will assume true (single instance) unless one of
/// the following scenarios is found:
///
/// 1. TERM_PROGRAM environment variable is a non-empty value. In this
/// case, we assume Void is being launched from a graphical terminal
/// session and you want a dedicated instance.
///
/// 2. Any CLI arguments exist. In this case, we assume you are passing
/// custom Void configuration. Single instance mode inherits the
...
/// The default value is `detect`.
///
/// Note that debug builds of Void have a separate single-instance ID
...
/// If `true` (default), then the Void GTK tabs will be "wide." Wide tabs
...
///
/// Launch Void with `env GTK_DEBUG=interactive void` to tweak Void's
/// CSS in real time using the GTK Inspector. Errors in your CSS files would
/// also be reported in the terminal you started Void from. See
...
/// escape sequences such as OSC 4. (Since Void 1.2.0, the previous
/// configuration `bold-is-bright` is deprecated and replaced by this
/// usage).
///
/// Available since Void 1.2.0.
...
/// keyprotocol=void:kitty`, however a bug in the implementation prevents it
/// from working properly. https://github.com/vim/vim/pull/13211 fixes this.
term: []const u8 = "xterm-void",
...
///
/// If the selected backend is not available on the platform, Void will
...
/// Control the auto-update functionality of Void. This is only supported
/// on macOS currently, since Linux builds are distributed via package
/// managers that are not centrally controlled by Void.
///
...
/// underlying protocols. To put it another way: Void doesn't explicitly
...
/// running Void version. If you download a pre-release version of Void
/// then this will be set to `tip` and you will receive pre-release updates.
/// If you download a stable version of Void then this will be set to
...
/// Set to true if Void was executed as xdg-terminal-exec on Linux.
...
test "handle bom in config files" {
...

    {
        ...
        try cfg.loadReader(
            alloc,
            &reader,
            "/home/void/.config/void/config.void",
        );
        try cfg.finalize();
...
/// configuration file is at `$XDG_CONFIG_HOME/void/config.void`.
///
...
pub fn finalize(self: *Config) !void {
    ...
    if (self.theme) |theme| {
        ...
        if (different) {
            ...
            // because it'll force the theme based on the Void theme.
    ...
    // short value because it can cause Void to quit before the first
...
/// Void in a CLI environment. We need this to change some behaviors.
...
/// If this is changed, you must also update void.h
...
pub const WorkingDirectory = union(enum) {
    ...
    test "WorkingDirectory parseCLI" {
        ...
        try wd.parseCLI(alloc, "~/projects/void");
        try testing.expectEqualStrings("~/projects/void", wd.path);
    ...
    test "WorkingDirectory finalize" {
        ...
        {
            var wd: Self = .{ .path = "~/projects/void" };
            try wd.finalize(alloc);
    ...
    pub fn init(self: *Keybinds, alloc: Allocator) !void {
        ...
        }
        {
            ...
            inline while (i <= end) : (i += 1) {
                ...
                // libvoid trigger API returns this one for the action,
        ...
        if (comptime builtin.target.os.tag.isDarwin()) {
            ...
            // void: cmd+g reserved for P1 grid-mode toggle (void upstream = Find Next).
...
/// See macos-icon
///
/// Note: future versions of Void can support a custom icon with
...
pub const WindowDecoration = enum(c_int) {
    ...
    pub const getGObjectType = switch (build_config.app_runtime) {
        .gtk => @import("gobject").ext.defineEnum(
            WindowDecoration,
            .{ .name = "VoidConfigWindowDecoration" },
        ),
...
test "working-directory expands tilde" {
    ...
    defer cfg.deinit();
    var it: TestIterator = .{ .data = &.{
        "--working-directory=~/projects/void",
    } };
    try cfg.loadIter(alloc, &it);
    try cfg.finalize();
